...
{ # Split data from vector Y into 2 bins in predefined ratio while preserving relative retios of
  ...
  if (nGroup>0 && nGroup!=nSamp) 
    stop("Error in msc.sample.split: Vectors 'Y' and 'group' have to have the same length")
  ...
  if (SplitRatio>=nSamp) 
    stop("Error in msc.sample.split: 'SplitRatio' parameter has to be i [0, 1] range or [1, length(Y)] range")
